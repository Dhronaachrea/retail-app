...
public class BetAmountDialogAdapter extends RecyclerView.Adapter<BetAmountDialogAdapter.BetAmountViewHolder> {
    ...
    public BetAmountDialogAdapter(Context context, ArrayList<FiveByNinetyBetAmountBean> listAmount, OnAmountSelectionListener listener) {
    ...
    @NonNull
    @Override
    public BetAmountDialogAdapter.BetAmountViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull BetAmountDialogAdapter.BetAmountViewHolder holder, int position) {
        if (Utils.getDeviceName().equalsIgnoreCase(AppConstants.DEVICE_V2PRO))
            holder.tvAmount.setTextSize(TypedValue.COMPLEX_UNIT_SP,11);
        ...
        if (listAmount.get(position).isSelected()) {
            ...
            listener.onAmountSelected(listAmount.get(position).getAmount());
        }
        else {
