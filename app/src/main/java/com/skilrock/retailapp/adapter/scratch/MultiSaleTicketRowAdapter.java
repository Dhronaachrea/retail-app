...
public class MultiSaleTicketRowAdapter extends RecyclerView.Adapter<MultiSaleTicketRowAdapter.MultiSaleTicketRowViewHolder> {


    private ChallanBookSelectionDialog dialog;
    private GetTicketStatusResponse.Game ticketAndStatusList = null ;
    ...
    public MultiSaleTicketRowAdapter(Context context) {
        this.context = context;
    }

    public MultiSaleTicketRowAdapter(Context context, boolean showTextWhite) {
        this.context = context;
        this.showTextWhite = showTextWhite;
    }
    public MultiSaleTicketRowAdapter( boolean showInvalidTickets,Context context) {
        this.context = context;
        this.showInvalidTickets = showInvalidTickets;
    }

    public MultiSaleTicketRowAdapter(Context context, boolean showWhiteText, GetTicketStatusResponse.Game ticketAndStatusList) {
        this.context = context;
        this.showTextWhite = showTextWhite;
        this.ticketAndStatusList = ticketAndStatusList;
    }
    public MultiSaleTicketRowAdapter(Context context, GetTicketStatusResponse.Game ticketAndStatusList, boolean showInvalidTickets) {
        this.context = context;
        this.showInvalidTickets = showInvalidTickets;
        this.ticketAndStatusList = ticketAndStatusList;
    }

    public MultiSaleTicketRowAdapter(Context context, MultiSaleTicketAdapter.RowAdapterCallBack callBack) {
        this.context = context;
    ...
    public void onBindViewHolder(@NonNull MultiSaleTicketRowViewHolder holder, int position) {
        ...
        Float amount = (float) (ticketAndStatusList.getTicketPrice());
        holder.tvTicketNumber.setText(ticketAndStatusList.getTicketAndStatusList().get(position).getTicketNumber());
        ...
        if(this.showTextWhite) {
            ...
            holder.tvInvalidTicketNo.setText(ticketAndStatusList.getTicketAndStatusList().get(position).getTicketNumber());
    ...
    public int getItemCount() {
        return ticketAndStatusList != null ? ticketAndStatusList.getTicketAndStatusList().size() : 0;
    }
    ...
    public class MultiSaleTicketRowViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.deleteBtn: {
                    try {
                        callBack.onClick(ticketAndStatusList.getTicketAndStatusList().get(getAdapterPosition()).getTicketNumber());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public void setTicketAndStatusListData( GetTicketStatusResponse.Game data) {
