...
import com.skilrock.retailapp.adapter.drawgame.ResultPreviewAdapter;
...
public class ResultPreviewActivity extends BaseActivity implements ResultPreviewAdapter.OnPrintClickListener{
    private RecyclerView rvResultPreview;
    private ResultPreviewAdapter previewAdapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (Utils.getDeviceName().equalsIgnoreCase(AppConstants.DEVICE_T2MINI)) {
            ...
            setContentView(R.layout.activity_result_preview_v2pro);
        } else {
    ...
    private void setInitialParameters() {
        ...
        if (bundle != null) {
            resultResponseBean = bundle.getParcelable("Result");
        }
    ...
    private void initializeWidgets() {
        ...
        previewAdapter = new ResultPreviewAdapter(this, resultResponseBean.getResponseData(), this);
    ...
    public void onPrintClick(int position) {
        if (position != -1) {
            Intent intent = new Intent(ResultPreviewActivity.this, PrintDrawGameActivity.class);
            intent.putExtra("PrintDataResult", resultResponseBean);
            intent.putExtra("Category", PrintDrawGameActivity.RESULT);
            intent.putExtra("item", position);
            startActivityForResult(intent, REQUEST_CODE_PRINT);
        }else {
            Utils.showErrorMessage(ResultPreviewActivity.this,getString(R.string.no_result_print));
        }
