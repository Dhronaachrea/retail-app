...
public class UserDetailFragment extends BaseFragment implements View.OnClickListener {
    ...
    private void setUserData(UserDetailResponseBean.Data data) {
        String mobileNumber = "";
        if (BuildConfig.app_name.equalsIgnoreCase(AppConstants.CAMEROON) || BuildConfig.app_name.equalsIgnoreCase(AppConstants.UNL_RETAIL)) {
            mobileNumber = data.getMobileNumber();
        } else {
            mobileNumber = data.getMobileNumber().substring(data.getMobileNumber().length() - 10);
        }

        //String mobileNumber = BuildConfig.app_name.equalsIgnoreCase(AppConstants.CAMEROON) ? data.getMobileNumber() : data.getMobileNumber().substring(data.getMobileNumber().length() - 10);
        String countryCode = data.getMobileCode();
